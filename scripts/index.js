...
const tbodyEl = document.querySelector('#activityTable tbody')

categories.forEach((category) => {
    const categoryDropdown = new Option(category, category)
    categorySelectorEl.appendChild(categoryDropdown)
})


function buildActivityRow(table, activity) {
...
categorySelectorEl.addEventListener('change', () => {
    ...
    if (!selectedValue) {
        loadTableRows(activityTableEl, activities);
        return;
    }

    const selectedActivity = activities.filter((activity) => activity.category == selectedValue)
    console.log(selectedActivity)

    clearTable(activityTableEl)

    loadTableRows(activityTableEl, selectedActivity)
});
...
function loadTableRows(table, activities) {
    ...
    activities.forEach((activity) => {
        buildActivityRow(table, activity)
    });
}
...
function clearTable(table) {
    ...
    // vv | alternative, more performant way: setting the tbody innerHTML to null/ or an empty string
